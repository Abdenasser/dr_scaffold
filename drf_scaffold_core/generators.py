...
from drf_scaffold_core.scaffold_templates import serializer_templates
from drf_scaffold_core.scaffold_templates import url_templates
...
class Generator(object):
    ...
    def __init__(self, appdir, model_name, fields):
        ...
        """      
        self.appdir = appdir
        self.MAIN_DIR = appdir.split("/")[0] if len(appdir.split('/'))>= 2 else './'
    ...
    def add_setup_imports(self, file_paths, matching_imports):
        """ 
        adds the bare minimum imports needed for each file
        """ 
        for i, f in enumerate(file_paths):
            file_api.set_file_content(f, matching_imports[i])

    def setup_files(self):
        """ 
        creates files if not exist, and adds appropriate imports 
        """ 
        files = (f"{self.appdir}/serializers.py", f"{self.appdir}/urls.py", f"{self.appdir}/models.py", f"{self.appdir}/admin.py", f"{self.appdir}/views.py")
        files_matching_imports = (serializer_templates.SETUP, url_templates.SETUP, model_templates.SETUP, admin_templates.SETUP, view_templates.SETUP)
